    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center border border-slate-100 group-hover:bg-cyan-50 group-hover:border-cyan-100 transition-colors duration-300 text-slate-400 group-hover:text-cyan-600">
           <FileText strokeWidth={1.5} size={22} className="group-hover:scale-110 transition-transform duration-300" />
        </div>
      ...
      <div className="mt-4 pt-4">
        {!isEditing ? (
          <h3
            className="font-medium text-slate-700 text-[15px] truncate w-full group-hover:text-cyan-700 transition-colors"
          >
            {note?.name}
          </h3>
        ) : (
